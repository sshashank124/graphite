...
impl RotScale3 {
    ...
    pub fn t(&self) -> Self {
        let m = self.0;
        Self::from_rows([m[0_usize][0], m[1_usize][0], m[2_usize][0]].into(),
                        [m[0_usize][1], m[1_usize][1], m[2_usize][1]].into(),
                        [m[0_usize][2], m[1_usize][2], m[2_usize][2]].into())
    }
}
...
{
    type Output = A3<A>;
    fn mul(self, o: A3<A>) -> A3<A> { A3::rep(o).zip(self.0, A3::dot) }
}
