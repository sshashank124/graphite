...
impl B {
    ...
    #[inline(always)] pub fn ordered(a: F2) -> B { if a[0] > a[1] { B(a.flip()) } else { B(a) } }

    #[inline(always)] pub fn lower(self) -> F { self.0[0] }
    #[inline(always)] pub fn upper(self) -> F { self.0[1] }

    #[inline(always)] pub fn bounds(self, t: F) -> bool { self.0[0] <= t && t <= self.0[1] }
    #[inline(always)] pub fn degen(self) -> bool { self.0[0] > self.0[1] }

    #[inline(always)] pub fn center(self) -> F { self.0.mean() }
    #[inline(always)] pub fn extent(self) -> F { self.0[1] - self.0[0] }
}
...
impl Mul<F> for B {
    type Output = B;
    #[inline(always)] fn mul(self, f: F) -> B { B::ordered(self.0 * f) }
}
