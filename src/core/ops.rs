...
macro_rules! op {
    ($trait:ident::$op:ident, *$type:ident) => {
        impl $trait for $type {
            type Output = $type;
            #[inline(always)] fn $op(self) -> $type { $type($trait::$op(self.0)) }
        }
    };
    ($trait:ident::$op:ident, *$lhs:ident -> *$rhs:ident -> $out:ident) => {
        impl $trait<$rhs> for $lhs {
            type Output = $out;
            #[inline(always)] fn $op(self, b: $rhs) -> $out { $out($trait::$op(self.0, b.0)) }
        }
    };
    ($trait:ident::$op:ident, $lhs:ident -> *$rhs:ident -> $out:ident) => {
        impl $trait<$rhs> for $lhs {
            type Output = $out;
            #[inline(always)] fn $op(self, b: $rhs) -> $out { $out($trait::$op(self, b.0)) }
        }
    };
    ($trait:ident::$op:ident, *$lhs:ident -> $rhs:ident -> $out:ident) => {
        impl $trait<$rhs> for $lhs {
            type Output = $out;
            #[inline(always)] fn $op(self, b: $rhs) -> $out { $out($trait::$op(self.0, b)) }
        }
    };
    ($trait:ident::$op:ident, *mut $lhs:ident -> *$rhs:ident -> ()) => {
        impl $trait<$rhs> for $lhs {
            #[inline(always)] fn $op(&mut self, b: $rhs) { $trait::$op(&mut self.0, b.0); }
        }
    };
    ($trait:ident::$op:ident, *mut $lhs:ident -> $rhs:ident -> ()) => {
        impl $trait<$rhs> for $lhs {
            #[inline(always)] fn $op(&mut self, b: $rhs) { $trait::$op(&mut self.0, b); }
        }
    };
}
...
macro_rules! cw_unary_op {
    ($array:ident, $trait:ident::$op:ident) => {
        ...
        {
            type Output = $array<A>;
            #[inline(always)] fn $op(self) -> Self::Output { $array::map(self, $trait::$op) }
        }
    };
}
...
macro_rules! cw_binary_assign_op {
    ($array:ident, $trait:ident::$op:ident) => {
        impl<A, B> $trait<$array<B>> for $array<A>
            where A: $trait<B>
        {
            #[inline(always)] fn $op(&mut self, b: $array<B>) { $array::zipi(self, b, $trait::$op) }
        }
    };
}
    ...
    ($array:ident, $trait:ident::$op:ident) => {
        impl<A, N> $trait<N> for $array<A>
            where N: Num,
                  A: $trait<N>
        {
            #[inline(always)] fn $op(&mut self, n: N) { $array::zipsi(self, n, $trait::$op) }
        }
    };
}
