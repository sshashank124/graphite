...
impl<A> A3<A> {
    #[inline(always)] pub const fn as_ref(&self) -> A3<&A>
    { A3(&self.0, &self.1, &self.2) }

    #[inline(always)] pub fn map<B>(self, f: impl Fn(A) -> B) -> A3<B>
    { A3(f(self.0), f(self.1), f(self.2)) }

    #[inline(always)] pub fn zip<B, C>(self, b: A3<B>, f: impl Fn(A, B) -> C) -> A3<C>
    { A3(f(self.0, b.0), f(self.1, b.1), f(self.2, b.2)) }

    #[inline(always)] pub fn zips<B, C>(self, b: B, f: impl Fn(A, B) -> C) -> A3<C> where B: Copy
    ...
    #[inline(always)] pub fn dot<B, C>(a: A3<A>, b: A3<B>) -> C
...
impl<A> A3<A> where A: Copy {
    ...
    #[inline(always)] pub fn a2a(a2: A2<A>, a: A) -> A3<A> { A3(a2[0], a2[1], a) }
...
{ #[inline(always)] pub fn mean(self) -> A { self.sum() * F::ONE_THIRD } }
...
impl<A> Product for A3<A> where Self: One + Mul<Output=Self> {
    #[inline(always)] fn product<It>(it: It) -> Self where It: Iterator<Item=Self>
...
{ #[inline(always)] fn of(aa: (A, A, A)) -> Self { A3(aa.0, aa.1, aa.2) } }

impl<A> ConvFrom<A3<A>> for (A, A, A)
{ #[inline(always)] fn of(aa: A3<A>) -> Self { (aa.0, aa.1, aa.2) } }

impl<A> ConvFrom<[A; 3]> for A3<A> where A: Copy
{ #[inline(always)] fn of(aa: [A; 3]) -> Self { A3(aa[0], aa[1], aa[2]) } }

impl<A> ConvFrom<A3<A>> for [A; 3]
{ #[inline(always)] fn of(aa: A3<A>) -> Self { [aa.0, aa.1, aa.2] } }

impl<A, B> Conv<A3<B>> for A3<A> where A: Conv<B> {
    #[inline(always)] fn conv(self) -> A3<B>
