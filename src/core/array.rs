...
macro_rules! cw_unary_op {
    ($trait:ident::$op:ident) => {
        ...
        {
            type Output = Arr<A, N>;
            #[inline(always)] fn $op(self) -> Self::Output { map(self, $trait::$op) }
        }
    };
}
cw_unary_op!(Neg::neg);
...
macro_rules! cw_binary_op {
    ($trait:ident::$op:ident) => {
        ...
        {
            type Output = Arr<C, N>;
            #[inline(always)] fn $op(self, b: Arr<B, N>) -> Self::Output
            { self.zip(b, $trait::$op) }
        }
    };
}
...
macro_rules! scalar_binary_assign_op {
    ($trait:ident::$op:ident) => {
        impl<A, S, const N: usize> $trait<S> for Arr<A, N>
            where A: $trait<S>,
                  S: Num
        { #[inline(always)] fn $op(&mut self, s: S) { self.zipsi(s, $trait::$op) } }
    };
}
...
{ #[inline(always)] fn index_mut(&mut self, i: I) -> &mut A { &mut self.0[i as usize] } }
